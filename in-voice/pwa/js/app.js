...
(function () {
  ...
  function nextNumero(tipo) {
    const nums = docs.filter((d) => d.tipo === tipo).map((d) => parseInt(d.numero, 10) || 0);
      ...
      wrap.innerHTML = `
        <div class="empty">
          ...
          <div class="empty-actions">
            <button class="btn primary" data-action="new-recibo">＋ Nuevo recibo</button>
            <button class="btn outline" data-action="new-cotizacion">Nueva cotización</button>
          </div>
        </div>`;
      return;
    }
  ...
  function renderEditor() {
    ...
    $('#editor-form').innerHTML = `
      ...
      <section class="fsec">
        ...
        <label>Cliente guardado
          <div class="client-pick">
            ...
            <button class="btn small outline" data-action="save-client" title="Guardar los datos actuales como cliente">💾 Guardar</button>
            <button class="btn small danger" data-action="del-client" title="Eliminar cliente seleccionado">🗑</button>
          </div>
        </label>
        <label>Proyecto
      ...
      <section class="fsec">
        ...
        <button class="btn outline small" data-action="add-item">＋ Agregar concepto</button>
      ...
      <section class="fsec">
        ...
        <button class="btn outline small" data-action="add-abono">＋ Agregar abono</button>
      ...
      <section class="fsec">
        ...
        <p class="hint">Estos datos aparecen junto al reloj ⏰ y la cajita 📦 del documento.</p>
  ...
  function itemRow(it, i) {
    ...
    <div class="item-row" data-index="${i}">
      ...
      <button class="icon-btn" data-action="del-item" data-index="${i}" title="Quitar">✕</button>
    </div>`;
  }
  function abonoRow(a, i) {
    ...
    <div class="item-row abono" data-index="${i}">
      ...
      <button class="icon-btn" data-action="del-abono" data-index="${i}" title="Quitar">✕</button>
    </div>`;
  }
    ...
    $('#settings-body').innerHTML = `
      ...
      <div class="client-list">
        ${clientes.length
          ...
          : '<p class="hint">Aún no hay clientes guardados. Guárdalos desde el editor de un documento (💾 Guardar).</p>'}
      </div>
    `;
  ...
  document.addEventListener('click', (e) => {
    ...
    switch (a) {
      ...
      case 'back': saveDoc(true); showList(); break;
      case 'save': saveDoc(false); break;
      case 'print': printDoc(); break;
      case 'email': sendEmail(); break;
      case 'share': shareDoc(); break;
      case 'delete':
        if (editing) { if (confirm('¿Eliminar este documento?')) { docs = docs.filter((d) => d.id !== editing.id); storeSet('mc_docs', docs); showList(); } }
        break;
  ...
  $$('.chip').forEach((c) => c.addEventListener('click', () => { listFilter = c.dataset.filter; renderList(); }));
